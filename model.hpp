...
public:
	...
	vector <float> my_strtok_f(char* str, char* delimeter) { //공백분리(float)
		vector <float> v;
		char* context;
		char* tok = strtok_s(str, delimeter, &context);

		while (tok != NULL) {
			v.push_back(atof(tok));// a to f
			tok = strtok_s(NULL, delimeter, &context);
		}
		return v;
	}

	vector <string> my_strtok_s(char* str, char* delimeter) { //공백분리(string)
		vector <string> v;
		char* context;
		char* tok = strtok_s(str, delimeter, &context);

		while (tok != NULL) {
			v.push_back(tok);// a to f
			tok = strtok_s(NULL, delimeter, &context);
		}
		return v;
	}

	vector <int> my_strtok_i(char* str, char* delimeter) { //공백분리(int)
		vector <int> v;
		char* context;
		char* tok = strtok_s(str, delimeter, &context);

		while (tok != NULL) {
			v.push_back(atoi(tok));// a to f
			tok = strtok_s(NULL, delimeter, &context);
		}
		return v;
	}
	...
	void loadObj(ifstream& fin) {
		...
		while (getline(fin, line)) {
			...
			if (line[0] == 'o' && line[1] == ' ') {
				...
				if (cnt > 0) {
					cnt_prev_vertex += objs[cnt - 1].v.size(); //누적 벡터수 추가
					cnt_prev_texture += objs[cnt - 1].vt.size(); //누적 벡터수 추가
					cnt_prev_normal += objs[cnt - 1].vn.size(); //누적 벡터수 추가
				}
				cnt += 1;
			}
			...
			if (line[0] == 'v' && line[1] == ' ') {
				...
				for (int i = 0; i < nVertexes; i++) {
					...
					p3i.d = { vi[0] - cnt_prev_vertex, vi[1] - cnt_prev_texture, vi[2] - cnt_prev_normal }; // vertext(65 - 64 = 1)
